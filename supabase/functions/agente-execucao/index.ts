  ...
  try {
    ...
    if (!body.jornada_id || !body.message) {
      return new Response(JSON.stringify({
        error: 'jornada_id and message are required'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const { data: jornada, error: jornadaError } = await supabase.from('jornadas_consultor').select('*').eq('id', body.jornada_id).single();
    if (jornadaError || !jornada) {
      throw new Error('Jornada não encontrada');
    }
    const { data: acoes, error: acoesError } = await supabase.from('kanban_cards').select('*').eq('jornada_id', body.jornada_id).order('created_at', {
      ascending: false
    });
    if (acoesError) {
...
6. **Auxiliai o usuario com passo a passo detalhado de elaboração, implementação execução e acompanhamento das ações do plano
7. **Dê feedback sobre as ações exeutadas pelo user
...
- Sempre informe quando uma ação automática foi executada`;
    const userMessage = body.file_context ? `${body.message}\n\n[Arquivos anexados: ${body.file_context}]` : body.message;
