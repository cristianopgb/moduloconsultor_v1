...
function normalizeSegmento(segmento?: string): string[] {
  ...
  // Saúde / clínicas
  if (seg.includes('clin') || seg.includes('saude') || seg.includes('saúde') || seg.includes('hospital') || seg.includes('consultorio') || seg.includes('consultório')) {
    normalized.push('saude', 'clinica', 'clinic');
  }
  if (seg.includes('ortoped') || seg.includes('ortopéd')) {
    normalized.push('ortopedia', 'clinica_ortopedica');
  }
...
export async function searchRelevantHints(
  ...
  try {
    ...
    // Extrair palavras para usar também como palavra-chave na query ilike
    const words = searchTerms.toLowerCase().split(/\s+/).filter(w => w.length > 3);
    const searchKeyword = words.length > 0 ? words[0] : (searchTerms.substring(0, 20).trim() || '');
    ...
    // Construir condição OR mais permissiva: combinar match por segmento OU busca textual no title/recommendations
    let orCondition = 'id.not.is.null';
    if (segmentosNormalizados.length > 0 && searchKeyword) {
      orCondition = `segmentos.cs.{${segmentosNormalizados.join(',')}},title.ilike.%${searchKeyword}% ,recommendations.ilike.%${searchKeyword}%`;
    } else if (segmentosNormalizados.length > 0) {
      orCondition = `segmentos.cs.{${segmentosNormalizados.join(',')}}`;
    } else if (searchKeyword) {
      orCondition = `title.ilike.%${searchKeyword}% ,recommendations.ilike.%${searchKeyword}%`;
    }

    const { data: hints, error } = await supabase
      .from('proceda_hints')
      // IMPORTANTE: incluir 'scenario' para calcular relevância textual mais abaixo
      .select('id, title, recommendations, scenario, segmentos, dominios, prioridade, uso_count, aceite_rate')
      .eq('ativo', true)
      .or(orCondition)
  ...
  const scoredHints = (hints as any[]).map((hint: any) => {
      let score = 0;
  ...
  scoredHints.sort((a: any, b: any) => b.score - a.score);
  ...
  topScores: topHints.map((h: any) => ({ title: h.title, score: h.score }))
    });
