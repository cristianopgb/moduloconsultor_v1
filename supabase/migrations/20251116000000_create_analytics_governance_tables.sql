/*
  ...
  1. New Tables
    - `semantic_dictionary`: Maps raw column names to canonical entities (vendor → Vendedor)
      - `id` (uuid, primary key)
      - `tenant_id` (uuid, nullable for system-wide entities)
      - `entity_type` (text: dimension/measure)
      - `canonical_name` (text, canonical entity name)
      - `synonyms` (jsonb, array of synonyms)
      - `description` (text)
      - `version` (integer, for versioning)
      - `created_at` (timestamptz)

    - `metrics_registry`: Single source of truth for metric definitions
      - `metric_id` (uuid, primary key)
      - `name` (text, unique identifier)
      - `display_name` (text, user-friendly name)
      - `formula_template` (text, calculation formula)
      - `required_columns` (jsonb, array of required columns)
      - `dependencies` (jsonb, array of other metric_ids)
      - `fallback_rules` (jsonb, fallback logic when data is missing)
      - `granularity` (text: row/group/aggregate)
      - `domain` (text: logistics/sales/hr/financial/generic)
      - `unit` (text, optional: %, $, etc)
      - `format` (text: number/percentage/currency/duration)
      - `created_at` (timestamptz)

    - `execution_lineage`: Tracks every execution for auditability
      - `exec_id` (uuid, primary key)
      - `exec_spec_hash` (text, hash of ExecSpec for caching)
      - `exec_spec` (jsonb, full ExecSpec)
      - `data_card_summary` (jsonb, DataCard summary)
      - `result_summary` (jsonb, result metadata)
    ...
    - `lineage_artifacts`: Links charts/tables/metrics to their source execution
      - `artifact_id` (uuid, primary key)
      - `exec_id` (uuid, references execution_lineage)
      - `artifact_type` (text: chart/table/metric/narrative)
      - `artifact_spec` (jsonb, full specification)
      - `position_in_report` (integer)
      - `created_at` (timestamptz)

    - `analytics_performance_log`: Performance metrics for monitoring
      - `request_id` (uuid, primary key)
      - `user_id` (uuid)
      - `conversation_id` (uuid)
      - `plan_time_ms` (integer)
      - `exec_time_ms` (integer)
      - `total_time_ms` (integer)
      - `confidence_score` (integer, 0-100)
      - `had_refinement` (boolean)
      - `token_cost_estimated` (integer)
      - `success` (boolean)
  ...
  2. Security
    ...
    - Admin-only access for semantic_dictionary and metrics_registry
    - User can read own lineage and performance logs
